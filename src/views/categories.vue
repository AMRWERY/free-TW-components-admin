                    ...
                    <tbody class="whitespace-nowrap">
                        ...
                        <template v-else>
                            <tr class="hover:bg-gray-50 cursor-pointer border-b"
                                v-for="(category, index) in paginatedCategories" :key="category.id">
...
const deleteCategoryIndex = ref<number | null>(null);
...
const editCategory = computed((): { name: string; category?: string; code?: string } | undefined => {
    if (editingIndex.value !== null) {
        const category = paginatedCategories.value[editingIndex.value];
        if (category) {
            return {
                name: category.name,
...
const handleDialogSubmit = async (data: { name: string }) => {
    if (dialogMode.value === "add") {
        ...
        if (category && category.id) {
            await categoryStore.updateCategory(category.id, data.name);
            if (!categoryStore.error) {
                triggerToast({
...
const openDeleteDialog = (index: number) => {
    deleteCategoryIndex.value = index;
    const category = paginatedCategories.value[index];
    if (category) {
...
const closeDeleteDialog = () => {
    ...
    deleteCategoryIndex.value = null;
    deleteCategoryName.value = "";
};

const confirmDelete = async () => {
    if (deleteCategoryIndex.value !== null) {
        const category = paginatedCategories.value[deleteCategoryIndex.value];
        if (category && category.id) {
